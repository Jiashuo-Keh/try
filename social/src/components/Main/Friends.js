  ...
  // },[friends]);
    ...
    const friend = friends.map((fri) => (
        <tr key={fri.id}>
          ...
          <img className='perimgage' style={{width:50,height:50}} src="https://p.qqan.com/up/2021-2/16137992352635419.jpg"  alt=""></img>
          <Button variant="outline-secondary" onClick={()=>deleteFriend(fri.id)} style={{float:"right"}}>x</Button>
          <ul>
            <span>{fri.name}</span>
            <br/>
